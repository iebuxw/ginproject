...
- **部署**：Docker Compose 编排，Go 多阶段构建（golang:1.18-alpine → alpine:3），API 代理到 go-app:8000
...
- 管理员权限 = 其所有角色绑定的所有 Menu.Permission 的并集
...
- 按功能模块分批提交，不同功能不混在一个 commit（如"修复字典操作列"和"新增管理员描述字段"分开提交）
